-- 1. Drop the foreign key constraint that requires a profile to exist in auth.users
-- This allows us to create student profiles using just their email.
ALTER TABLE profiles DROP CONSTRAINT profiles_id_fkey;

-- 2. Create the Registration RPC function to securely handle unauthenticated registrations
...
BEGIN
    ...
    IF FOUND THEN
        -- We just use the existing profile ID, and use their existing name/RRN for the response
        p_name := v_existing_name;
        p_reg_number := v_existing_reg_number;
    ELSE
        -- Generate a new profile ID and insert
        v_profile_id := gen_random_uuid();
        INSERT INTO profiles (id, name, email, gender, phone, registration_number, role)
        VALUES (v_profile_id, p_name, p_email, p_gender, p_phone, p_reg_number, 'STUDENT');
    END IF;

    -- 6. Check for duplicate registration (same user, same sub-event)
    IF EXISTS (
